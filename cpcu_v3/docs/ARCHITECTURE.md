...
| `model.py`     | Loads the 12-feature CSV. StratifiedGroupKFold(5, seed=33). StandardScaler. SVM RBF C=10, gamma='scale', class\_weight='balanced', probability=True. Saves `model.pkl` + `model.pkl (combined checkpoint)`. |
...
If `/opt/cpcu/models/{model.pkl, model.pkl (combined checkpoint)}`
...
`.pkl` degrades the system to a calibration / smoke-test rig
without compromising safety.

### 7.9 Model deployment workflow

The trained `.pkl` artefacts are not in this repo. To deploy:

1. On the team's Windows machine, run `feature_ex.py` (produces
   `features_200hz_segmented.csv`) and then `model.py` (produces
   `model.pkl` + `model.pkl (combined checkpoint)`).
2. SCP both files to the Pi: `scp *.pkl pi@<ip>:/opt/cpcu/models/`
...
Process 1: cpcu_dsp.py (Cores 1-2, Python)
  ...
  |- Loads SVM model + scaler from .pkl
...
Install targets:
  ...
  /opt/cpcu/models/       — model.pkl + scaler (operator-deployed)
