...
##
##  ROLE
##    Single entry point for every operation on the system: Pi
##    bring-up, compile, install, test, calibrate, run, stop. Wraps
##    the underlying scripts and binaries so users never have to know
##    about cmake / systemd / tmux / capabilities directly.
##
##  DEPENDENCIES
##    config/gestures.json     : authoritative gesture + servo + group
##                               + audio config; many commands edit it.
##    config/runtime.json      : compile-defaults shadow; written by
##                               configure / set-pca-channel / etc.
##    scripts/*.sh             : per-area implementations (run_tests,
##                               configure, audio_helper, etc.) — this
##                               file invokes them, never vice versa.
##    cpcu_kernel + binaries   : the runtime stack installed under
##                               /opt/cpcu/bin by `build`.
##
##  DOWNSTREAM
##    Operators                : the entire user-facing surface.
##    systemd                  : cpcu.service / cpcu_ws.service unit
##                               files are generated by install-*.
##    cpcu_tui / cpcu_ws       : preflight hooks here drop digest
##                               files in /tmp that those processes
##                               render on their respective dashboards.
##
##  CROSS-MODULE EFFECTS
##    - Any new subcommand needs three things: a cmd_*/run_* handler,
##      a dispatch case near the bottom, and a help block in
##      print_v3_help or cmd_help so it shows up in the reference.
##    - Adding or removing flags from tui/collect/signal also requires
##      updating the flag-combination table in print_v3_help — it's
##      the only place users discover what's combinable.
##
##  ════════════════════════════════════════════════════════════════════
...
##      ./launch.sh test-sw                Software-only tests
...
cat << 'HELPEOF'
  ...
  launch.sh — InfiniTech CPCU control script
  ════════════════════════════════════════════════════════════════════
  This is the ONLY script you need. Every operation goes through
  ./launch.sh <command> [flags]. Run ./launch.sh menu for an
  interactive picker, or ./launch.sh help <command> for per-command
  detail.
  ════════════════════════════════════════════════════════════════════

  THE CORRECT MINIMAL SEQUENCE  (fresh git clone on a Pi 5)
  ─────────────────────────────────────────────────────────────
    git clone <repo> && cd <repo>
    ./launch.sh setup                       # base config + reboot
    sudo reboot
    ./launch.sh vendor                      # fetch Mongoose for web UI
    ./launch.sh build                       # compile + install
    ./launch.sh check                       # verify everything ready
    ./launch.sh test-ipc                    # cross-check C↔Python IPC
    ./launch.sh test-pca                    # tune servo MIN/MAX/BIAS
    ./launch.sh calibrate --operator $USER  # rest noise + velocity
    export CPCU_OPERATOR=$USER              # carry through to runs
    ./launch.sh tui --audio --with-ws       # daily operation
  ─────────────────────────────────────────────────────────────

  FULL ORDER OF OPERATIONS  (every step, in dependency order)
  ─────────────────────────────────────────────────────────────
   ...
   └────────────────────────────────────────────────────────────┘
   ┌── Stage 3 — Verify subsystems before going live ───────────┐
   │  10. ./launch.sh test-sw          software-only tests      │
   │  11. ./launch.sh test-ipc         + IPC offset validation  │
   │  12. ./launch.sh test-hw          + Pi hardware probes     │
   │  13. ./launch.sh test-nrf         NRF24L01+ self-test      │
   │  14. ./launch.sh test-pca         interactive servo cal    │
   │  15. ./launch.sh test-signal-demo signal TUI (synthetic)   │
   │  16. ./launch.sh test-signal      signal TUI (live BSAU)   │
   │  17. ./launch.sh test-safety-demo fault injection demo     │
   └────────────────────────────────────────────────────────────┘
   ┌── Stage 4 — Configure (only if defaults need changing) ────┐
   │  18. ./launch.sh configure --show      see compile knobs   │
   │  19. ./launch.sh configure --reset --runtime  regen JSON   │
   │  20. ./launch.sh show-config           print full config   │
   └────────────────────────────────────────────────────────────┘
   ...
   │  23. ./launch.sh calibrate --operator NAME                 │
   │         → models/dynamic_noise_thresholds_<NAME>.json      │
   │         → models/velocity_map_<NAME>.json                  │
   │         Default operator profile is NEVER overwritten.     │
   │  24. export CPCU_OPERATOR=NAME    carry through tui/ws     │
   └────────────────────────────────────────────────────────────┘
   ┌── Stage 6 — Customise gestures / motors / audio (optional)─┐
   │  25. ./launch.sh add-group  NAME      new classifier       │
   │  26. ./launch.sh add-motor  NAME CH   new servo            │
   │  27. ./launch.sh add-gesture [group]  guided wizard        │
   │  28. ./launch.sh set-model  PATH      pick .pkl            │
   │  29. ./launch.sh audio voice          enable voice cues    │
   │  30. ./launch.sh show-config          verify changes       │
   └────────────────────────────────────────────────────────────┘
   ┌── Stage 7 — Run the live system ───────────────────────────┐
   │  31. ./launch.sh tui --audio --with-ws   daily operation   │
   ...
   │  32. ./launch.sh install-service       kernel as systemd   │
   │  33. ./launch.sh install-ws-service    web dashboard       │
   └────────────────────────────────────────────────────────────┘

  ENVIRONMENT VARIABLES
  ─────────────────────────────────────────────────────────────
    CPCU_OPERATOR=NAME   default operator for tui/collect/calibrate.
                         Overridden by an explicit --operator NAME on
                         any command. Reading order in cpcu_dsp.py:
                         flag > env var > "default".
    CPCU_UART_DEBUG=PATH UART device for the 1 kHz raw stream
                         (typically /dev/ttyAMA0). Enables tui --uart.
    CPCU_HOST_LAN_IP=IP  used by ws when the Pi is USB-tethered and
                         needs to advertise a host-PC tunnel target.

  ═══════════════════════════════════════════════════════════════════
  COMMAND REFERENCE  (every command, every flag combination)
  ═══════════════════════════════════════════════════════════════════
  ...
  ─────────────────────────────────────────────────────────────
    ...
    ./launch.sh setup-audio                   I2S DAC + speaker
                                                (sudo; reboot if dts changed)
    ./launch.sh setup-uart                    enable UART debug pin
                                                (sudo; reboot)
    ./launch.sh vendor                        fetch Mongoose (web srv)
    ./launch.sh build                         compile + install
    ./launch.sh build --clean                 wipe build/ first
    ./launch.sh generate-cues                 voice .wav files
    ./launch.sh check                         verify readiness (no run)
    ./launch.sh grant-caps                    re-apply RT capabilities
                                                (CAP_SYS_NICE, IPC_LOCK)

  RUNNING THE SYSTEM
  ─────────────────────────────────────────────────────────────
  The tui/collect/signal commands all bring up cpcu_kernel inside a
  tmux session and add per-command tool windows. Flags are
  combinable in any order.

    ./launch.sh tui                           plain TUI (KERNEL + SHELL + TUI)
    ./launch.sh tui --audio                   + voice/tone feedback window
    ./launch.sh tui --uart                    + UART stream to host PC
    ./launch.sh tui --with-ws                 + web dashboard window
    ./launch.sh tui --operator NAME           load operator's calibration
    ./launch.sh tui --audio --with-ws         common combo (audio + web)
    ./launch.sh tui --audio --uart --with-ws --operator NAME
                                              everything on, named profile

    ./launch.sh ws                            web dashboard only
                                                (auto-runs setup/build/vendor
                                                 if anything is missing)
    ./launch.sh ws --bind ws://127.0.0.1:8765 loopback-only (no LAN)
    ./launch.sh ws --static DIR               override web/ directory

    ./launch.sh kernel                        kernel only, foreground
                                                (systemd path; no UI)
    ./launch.sh collect                       dataset capture TUI
    ./launch.sh collect --with-ws             + web dashboard window
    ./launch.sh collect --operator NAME       capture under operator tag
    ./launch.sh signal                        live signal TUI (BSAU)
    ./launch.sh signal --with-ws              + web dashboard window
    ./launch.sh smoother                      servo motion exerciser
                                                (needs kernel running)
    ./launch.sh pca                           direct PCA9685 (no kernel)
                                                alias for test-pca
    ./launch.sh nrf [args]                    NRF self-test
    ./launch.sh menu                          interactive picker

    ./launch.sh attach                        re-attach last tmux session
    ./launch.sh stop                          stop (safe servo shutdown,
                                                kills tmux + cpcu_ws)

  TESTING (each test command runs its own phase plus all earlier)
  ─────────────────────────────────────────────────────────────
    ./launch.sh test-sw                       Phase 1: software only
                                                codec, safety FSM, smoother,
                                                DSP, runtime, JSON, editor
    ./launch.sh test-ipc                      Phase 1+2: also IPC bridge
                                                C↔Python offset cross-check
    ./launch.sh test-hw                       Phase 1+2+3: also Pi probes
                                                SPI, I²C, PCA9685, isolcpus,
                                                CPU temp
    ./launch.sh test-pca                      interactive servo TUI
                                                ← TUNE MIN/MAX/BIAS HERE
  ...
  CALIBRATION & TUNING
  ─────────────────────────────────────────────────────────────
  Operator workflow: every operator gets their own JSON files. The
  "default" operator's files are Aleyna's canonical set and are
  NEVER overwritten — even calibrating "default" forces a suffixed
  filename. Re-tag an operator with --operator and the files appear
  with _<name>.json suffixes.

    ./launch.sh grip-tune                     gripper firmness wizard
    ./launch.sh calibrate                     rest noise + velocity
                                                (operator = $CPCU_OPERATOR
                                                 or "default")
    ./launch.sh calibrate --operator NAME     per-operator profile
                                                → dynamic_noise_thresholds_<NAME>.json
                                                → velocity_map_<NAME>.json
    ./launch.sh calibrate --rest-only         noise floor only
                                                (skips velocity wizard)
    ./launch.sh calibrate --vel-only          velocity preference only
                                                (skips rest recording)
    ./launch.sh calibrate --duration SEC      override rest recording length
                                                (default: 10s)
    ./launch.sh calibrate --operator NAME --rest-only --duration 15
                                              combined: longer rest recording
                                                for one operator

  EMG CHANNELS
  ─────────────────────────────────────────────────────────────
  Match the number of channels listed in gestures.json to the model.
  set-channels validates this on every call.
  ...
  SERVO MOTORS
  ─────────────────────────────────────────────────────────────
  Names are referenced from gestures throughout the system — renames
  cascade automatically. The current 6-servo arm is named
  Base/Elbow/Forearm/Wrist1/Wrist2/Gripper.

    ./launch.sh add-motor    NAME CH         add on PCA channel CH (0-15)
    ./launch.sh edit-motor   NAME            edit limits + neutral
                                                (validates against compile-time
                                                 hardware envelope)
    ./launch.sh rename-motor OLD NEW         rename (cascades to gestures)
    ./launch.sh remove-motor NAME            remove a servo
                                                (asks before purging
                                                 dependent gestures)
    ./launch.sh set-pca-channel NAME CH      re-wire to PCA ch CH
                                                (validates 0-15 +
                                                 uniqueness; preflight
                                                 auto-syncs runtime.json)

  GESTURE GROUPS  (one classifier per group; right_arm + left_arm)
  ─────────────────────────────────────────────────────────────
    ./launch.sh show-config                  show all groups + gestures
    ./launch.sh show-gestures                alias of show-config
    ./launch.sh add-group    NAME            create (asks EMG + model)
    ./launch.sh remove-group NAME            delete entire group
    ./launch.sh rename-group OLD NEW         rename a group

  GESTURES  (each gesture belongs to one group)
  ─────────────────────────────────────────────────────────────
    ./launch.sh add-gesture                  add to first group (wizard)
    ./launch.sh add-gesture GROUP            add to specific group
    ./launch.sh edit-gesture NAME            change servo mapping
    ./launch.sh rename-gesture OLD NEW       rename gesture
    ./launch.sh remove-gesture GROUP NAME    delete gesture from group
  ...
  MODEL MANAGEMENT
  ─────────────────────────────────────────────────────────────
    ./launch.sh set-model models/arm.pkl     set active model
    ./launch.sh set-model                    list available models
                                                (current arm.pkl is a
                                                 4-class RandomForest:
                                                 ext, flex, hand, rest)

  COMPILE-TIME CONFIGURATION  (safety thresholds, NRF channel, etc.)
  ─────────────────────────────────────────────────────────────
  For RUNTIME tunables (servo limits, smoother, grip) edit
  config/runtime.json or use the TUI's edit mode (press 'e' on the
  CONFIG page). These commands change COMPILE-TIME values and
  require a rebuild after.
    ...
    ./launch.sh configure --reset --runtime  also regen runtime.json
    ./launch.sh configure --<knob>           show one current value
    ./launch.sh configure --<knob> VALUE     set one value
       Available knobs:
         --radio-timeout       silence (ms) → RUNNING → DEGRADED
         --radio-safe          DEGRADED (ms) → SAFE
         --boot-grace          cold-start grace before radio fault
         --vbat-low / --vbat-crit       battery LOW / CRITICAL (V)
         --thermal-warn / --thermal-crit thermal WARN/CRIT (°C)
         --i2c-max             consecutive I²C failures → SAFE
         --ring-overflow       ring overflows before fault
         --nrf-channel         BSAU NRF channel (0-125; MUST match BSAU)

  RUNTIME RELOAD  (apply config without full restart)
  ─────────────────────────────────────────────────────────────
  ...
  ─────────────────────────────────────────────────────────────
    ...
    ./launch.sh tui --uart                   run with UART debug
       Host (Linux):   python3 scripts/uart_monitor.py --port /dev/ttyUSB0
       Host (Windows): python3 scripts/uart_monitor.py --port COM3 --log data.csv

  SERVICES  (auto-start at boot)
  ─────────────────────────────────────────────────────────────
    ./launch.sh install-service              kernel as systemd unit
    ./launch.sh install-ws-service           web dashboard as systemd unit
    ./launch.sh grant-caps                   re-apply RT capabilities

  META
  ─────────────────────────────────────────────────────────────
    ./launch.sh help                         this message
    ./launch.sh help <command>               per-command detail
    ./launch.sh version                      script + IPC version
    ./launch.sh menu                         interactive picker

  COMMON RECIPES
  ─────────────────────────────────────────────────────────────

    # daily operation, single operator:
    export CPCU_OPERATOR=$USER
    ./launch.sh tui --audio --with-ws

    # full setup of a fresh Pi from a freshly cloned repo:
    ./launch.sh setup && sudo reboot
    ./launch.sh setup-audio                          # if voice cues wanted
    ./launch.sh vendor                               # web dashboard
    ./launch.sh build                                # compile + install
    ./launch.sh generate-cues                        # voice .wav files
    ./launch.sh check                                # verify readiness
    ./launch.sh test-ipc                             # software + IPC sanity
    ./launch.sh test-pca                             # tune servo MIN/MAX
    ./launch.sh calibrate --operator $USER           # operator profile
    export CPCU_OPERATOR=$USER
    ./launch.sh tui --audio --with-ws                # daily run

    # add a servo motor + use it in a gesture:
    ./launch.sh add-motor    Thumb 6
    ./launch.sh edit-motor   Thumb
    ./launch.sh add-gesture                          # wizard asks motors
    ./launch.sh collect                              # record training data
    ./launch.sh set-model    models/new.pkl          # after retraining
    ./launch.sh reload

    # multi-group setup (right arm + left arm, the default in v3):
    ./launch.sh show-config                          # confirm both groups
    ./launch.sh add-gesture right_arm                # add to one group
    ./launch.sh remove-gesture right_arm flex        # remove from one

    # calibrate for a new operator and switch to them:
    ./launch.sh calibrate --operator ali
    export CPCU_OPERATOR=ali
    ./launch.sh tui --audio                          # uses ali's profile

    # revert an operator to default thresholds (just delete their file):
    rm models/dynamic_noise_thresholds_ali.json
    # next ./launch.sh tui --operator ali falls back to "default"

    # rename things (all gesture refs follow automatically):
    ./launch.sh rename-motor Gripper Claw

    # switch audio mode without restarting the kernel:
    ...
    # UART debug to laptop (after setup-uart + reboot):
    ./launch.sh tui --uart
...
cmd_version() {
    # Single source of truth: the IPC_VERSION word that cpcu_ipc.h
    # commits to. Anything that links against the IPC layout reports
    # the same number, so a misversioned binary surfaces here too.
    local ipcv=""
    if [ -r "${CPCU_ROOT:-.}/include/cpcu_ipc.h" ]; then
        ipcv=$(grep -oE 'IPC_VERSION[[:space:]]+0x[0-9a-fA-F]+' \
                 "${CPCU_ROOT:-.}/include/cpcu_ipc.h" 2>/dev/null \
               | head -1 | awk '{print $NF}')
    fi
    if [ -n "${ipcv}" ]; then
        echo "launch.sh (InfiniTech CPCU)  IPC_VERSION=${ipcv}"
    else
        echo "launch.sh (InfiniTech CPCU)"
    fi
}
...
# ══════════════════════════════════════════════════════════════════════
#  EXTENDED COMMANDS — gestures.json, runtime.json, operator-config
# ══════════════════════════════════════════════════════════════════════
## ═══════════════════════════════════════════════════════════════════
##  Adds: gesture / group / motor / channel / audio / model commands
##        plus calibrate / grip-tune / set-pca-channel / reload.
##  These were originally a separate launch_additions.sh; merged in
##  place. The dispatch cases live near the bottom of this file.
## ═══════════════════════════════════════════════════════════════════
...
case "${MODE}" in
    ...
    # gestures.json / runtime.json / operator-config commands
    setup-audio)            cmd_setup_audio "$@" ;;
    setup-uart)             cmd_setup_uart "$@" ;;
    grip-tune)              run_grip_tune "$@" ;;
    calibrate)              run_calibrate "$@" ;;
