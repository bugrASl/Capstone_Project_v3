...
tmux_create_with_kernel() {
    ...
    local kernel_script="${LOG_DIR}/kernel_holder.sh"
    mkdir -p "${LOG_DIR}"
...
cleanup() {
    ...
    # Holder scripts now live under ${LOG_DIR}/ (stable filenames,
    # overwritten on each launch). They don't accumulate and never
    # need to be rm'd here — that race was the source of phantom
    # WS / KERNEL panes.
    kernel_stop_background_fallback
}
...
ws_window_cmd() {
    local sd
    sd="$(ws_static_dir)"
    local ws_script="${LOG_DIR}/ws_holder.sh"
    mkdir -p "${LOG_DIR}"
    cat > "${ws_script}" << WSHOLDER_EOF
#!/bin/bash
# Auto-generated by launch.sh. Rewritten on every tui --ws launch.
# Don't exec — we want the exit code visible if cpcu_ws crashes.
"${BIN_DIR}/cpcu_ws" --static "${sd}"
rc=\$?
echo
printf '\\033[31m========================================\\033[0m\\n'
printf '\\033[31m[cpcu_ws exited with code %d]\\033[0m\\n' "\$rc"
if [ \$rc -ne 0 ]; then
    printf '\\nLikely causes:\\n'
    printf '  - IPC shared memory not yet ready (start KERNEL first)\\n'
    printf '  - /dev/shm/cpcu_ipc missing or zero-size\\n'
    printf '  - ws_static dir missing: ${sd}\\n'
    printf '  - port 8765 already in use (ss -ltn | grep 8765)\\n'
fi
printf 'This pane stays open for inspection.\\n'
printf 'Ctrl-b 1 = SHELL window, Ctrl-b d = detach.\\n\\n'
exec bash --login
