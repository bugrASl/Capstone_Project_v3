...
ws_window_cmd() {
    local sd
    sd="$(ws_static_dir)"
    # Write the WS holder script to ${LOG_DIR}/ws_holder.sh — a stable
    # path that survives the EXIT trap. Earlier we used mktemp under
    # /tmp, but the cleanup trap rm'd the file between tmux's
    # new-window queueing and the shell actually exec'ing it,
    # producing a phantom pane and the misleading "tmux refused" error.
